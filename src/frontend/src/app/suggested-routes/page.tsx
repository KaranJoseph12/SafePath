...
const RouteCard = ({ route }: { route: RouteInfo }) => {
  ...
  const color = isSafe
    ? { ring: 'ring-green-600/30', head: 'text-green-700', bar: 'bg-green-600', pill: 'bg-green-50 text-green-700', btn: 'bg-green-600 hover:bg-green-700' }
    : { ring: 'ring-amber-500/30', head: 'text-amber-700', bar: 'bg-amber-500', pill: 'bg-amber-50 text-amber-700', btn: 'bg-amber-500 hover:bg-amber-600' };

  return (
    <section className={`card ${color.ring}`}>
      <header className="mb-2 flex items-center gap-2">
        <span className={`inline-flex h-6 w-6 items-center justify-center rounded-full ${isSafe ? 'bg-green-100' : 'bg-amber-100'}`}>
          {isSafe ? '🛡️' : '⚡'}
        </span>
      ...
      <div className="mt-3">
        ...
        <div className="progress mt-1">
          <div className={`h-full rounded-[10px] ${color.bar}`} style={{ width: `${score}%` }} />
        </div>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {highlights.map((h, i) => (
            <span key={i} className={`chip ${color.pill}`}>
              <span className="mr-1">{h.icon}</span>{h.label}
            </span>
          ))}
        </div>
      </div>
  ...
  return (
    <main className="mx-auto max-w-screen-sm p-4 pb-24">
      ...
      <div className="map-card relative mb-4">
        ...
        <div className="absolute left-4 bottom-4 rounded-full bg-black/80 px-2 py-1 text-white">📍</div>
      </div>
