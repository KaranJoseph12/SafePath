                      ...
                      {hazards.slice(0, 5).map((hazard) => (
                        <div key={hazard.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                          <div className="flex justify-between items-start">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <div className="font-semibold text-primary-dark capitalize">
                                  {(hazard.type || hazard.hazardType || 'Unknown')?.replace('_', ' ')}
                                </div>
                                <div className="text-sm text-gray-600">{hazard.description || 'No description available'}</div>
                                <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
                                  <span>📅 Reported {hazard.createdAt ? new Date(hazard.createdAt).toLocaleDateString() : 'Unknown date'}</span>
                                  {hazard.distance && (
                            ...
                            <span className={`px-2 py-1 rounded text-xs font-medium ${getSeverityBadgeColor(hazard.severity || 'unknown')}`}>
                              {(hazard.severity || 'Unknown')} Risk
                            </span>
                          </div>
                        </div>
                      ))}
